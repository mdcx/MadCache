 ...
 */
define( 'MC_ROOT_PATH', dirname( __FILE__ ) );
define( 'MC_DATA_PATH', MC_ROOT_PATH . '/data' );
...
class MadCache {
	...
	const CACHE_TAG = false; // This sets whether or not you want a <!-- coment --> with some data in it
	const CACHE_PATH = MC_DATA_PATH; // path to cache folder
	const CACHE_MAX_HASH = 3; // The max # of folders to seperate cache files into. Don't change unless you completely wipe the entire cache structure.
	...
	const CACHE_LOG = true; // For debug purposes.
	...
	/**
	 * Put all sub.* into a single host folder
	 */
	const CACHE_NO_SUBDOMAINS = false;
	...
	private $_expire_override = false;
	...
	function initialize()
	{
		...
		$this->_cache_path = self::CACHE_PATH . '/' . $this->get_host_folder();
	...
	public function set_expire_override( $time )
	{
		if ( !is_numeric( $time ) )
			return;
		$this->_expire_override = $time;
	}

	public function get_host_folder()
	{
		$folder = strtolower( $_SERVER['HTTP_HOST'] );
		if ( substr( $folder, 0, 4 ) == 'www.' )
			$folder = substr_replace( $folder, '', 0, 4 );
		$this->_host_folder = $folder;
		if ( preg_match( "|[^A-Za-z0-9\.\/]|", $this->_host_folder, $matches ) || $this->_host_folder == "0" || $this->_host_folder == "" )
			$this->_host_folder = "garbage_bin";
		if ( self::CACHE_NO_SUBDOMAINS === TRUE && substr_count( $this->_host_folder, '.' ) > 2 )
		{
			$purl = explode( '.', $this->_host_folder );
			$purl = array_slice( $purl, -2 );
			$this->_host_folder = implode( '.', $purl );
		}
		return $this->_host_folder;
	}

	public function start_cache( $EXPIRE, $KEY=NULL, $GROUP = NULL, $CHECK_ONLY=FALSE )
	{
		...
		if ( $this->_default_key !== true && strpos( $KEY, '::' ) !== false )
		{
			list($this->_cache_group, $this->_original_key) = explode( '::', $KEY );
			$this->_cache_path .= '/' . $this->_cache_group;
			if ( !is_dir( $this->_cache_path ) )
				@mkdir( $this->_cache_path, self::CACHE_STORAGE_PERM );
		}
	...
	public function cache_expire( $how="key", $what=NULL )
	{
		...
		if ( $how == "key" )
		{
			...
			if ( file_exists( $cache_file ) && unlink( $cache_file ) )
				$this->cache_log( "Manually expired $cache_file", TRUE );
		}
		else if ( $how == "host" )
		{
...
-->\n";
	...
	public function cache_check()
	{
		if ( file_exists( $this->_cache_file ) )
		{
			...
			elseif ( $this->_expire_override !== false && $this->_cache_mtime < $this->_expire_override )
			{
				$this->cache_log( "Cache override was activated: $this->_cache_mtime < $this->_expire_override (real: $this->_expire)." );
				return true;
			}
			else
			{
	...
	public function make_hash( $key, $hash_type=NULL )
	{
		if ( $hash_type == NULL )
	...
	public function read_cache( $echo_content = TRUE )
	{
		...
		if ( $this->_cache_method == "diskread" )
		{
			...
			if ( self::CACHE_USE_GZIP == TRUE )
			{
				...
				{
					$this->cache_log( "Failed to inflate the cache. Size: " . strlen( $this->_cache_content ) );
				}
	...
	public function make_path( $tmp_key=NULL )
	{
		if ( $tmp_key === NULL )
		{
			$tmp_key = $this->_key;
		...
		if ( $check == TRUE )
		{
			...
			else if ( self::CACHE_CHECK_DIR == "ONLOAD" )
			{
				if ( !is_dir( $folder ) )
				{
					$mk = @mkdir( $folder, self::CACHE_STORAGE_PERM, TRUE );
					$pretty_folder = str_replace(self::CACHE_PATH,'',$folder);
					if ( !$mk)
						$this->cache_log( "Directory struct failed! (onload) $folder", true );				
					else
						$this->cache_log( "Directory struct created (onload) $folder" );					
				}
			}
		}
		return $folder;
	...
	public function write( $path, $what, $mode = "w" )
	{
		return $this->_write_flock( $path, $what, $mode );
	}
