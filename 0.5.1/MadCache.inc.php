<?php
 ...
 class MadCache
 {
    ...
    const CACHE_TYPE = "MANUAL"; // Use output buffering to cache the parts you want. Or MANUAL to start the cache and store variables/data
    const CACHE_TAG = FALSE; // This sets whether or not you want a <!-- coment --> with some data in it
    const CACHE_PATH = "/tmp/MadCache/0.5.1"; // path to cache folder
    ...
    const CACHE_PER_HOST = FALSE; // If this is true, you can put different caches in diff folders by setting the key like FOLDER_NAME::whatever ..
    ...
    function initialize()
    {
		...
		$calling_script = !isSet($_SERVER['DOCUMENT_ROOT']) || empty($_SERVER['DOCUMENT_ROOT']) || strlen($_SERVER['DOCUMENT_ROOT']) < 2 ? "no_doc_root" : $_SERVER['DOCUMENT_ROOT'];
    ...
    public function start_cache($EXPIRE,$KEY=NULL,$CHECK_ONLY=FALSE)
    {
        ...
        if(self::CACHE_PER_HOST == TRUE)
        {
            if($this->_default_key == TRUE || !strstr($this->_original_key,"::"))
            {
				...
				if ( strpos($this->_host_folder, "www") !== FALSE ) $this->_host_folder = str_replace("www.", "", $this->_host_folder);
            }
            else
            {
...
-->\n";
	...
	register_shutdown_function('msg', $this->cache_tag_out);	
    }
	...
	public function cache_this($content=null)
    {
    ...
    private function cache_log($what,$override = FALSE)
    {
        ...
        if(self::CACHE_LOG == TRUE || $override == TRUE)
        {
            ...
            $log_line = "{$host_ident}$this->_pretty_time|$this->_key|$what\n";
